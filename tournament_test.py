...
def testReportMatches():
    ...
    reportMatch(False, 1, id1, id2)
    reportMatch(False, 1, id3, id4)
    standings = playerStandings()
    for (i, n, w, m) in standings:
        if m != 1:
            raise ValueError("Each player should have one match recorded.")
        if i in (id1, id3) and w != 2:
            raise ValueError("Each match winner should have one win recorded.") # To distinguish Draw from Win we count 2 points each win and 1 point a draw
...
def testPairings():
    ...
    reportMatch(False, 1, id1, id2)
    reportMatch(False, 1, id3, id4)
    reportMatch(False, 1, id5, id6)
    reportMatch(False, 1, id7, id8)
    pairings = swissPairings()
    if len(pairings) != 4:
        raise ValueError(
            "For eight players, swissPairings should return 4 pairs. Got {pairs}".format(pairs=len(pairings)))
